...
| Storage assumptions remain visible before pilot use | `tests/test_api_integration.py`, `tests/test_api_database.py`, `scripts/live-api-workflow-smoke.py`, `scripts/live-security-workflow-smoke.py`, `scripts/static-demo-smoke.js`, `tests/test_web_static.py`, `docs/RELEASE_CHECKLIST.md` | `/admin/storage/postgresql-readiness`, `PostgresMigrationReadinessResponse`, `storage_readiness`, `postgresql_schema_inventory`, `postgresql_disposable_ci`, `MICROSCORE_STORAGE_BACKEND`, `production_ready`, `temporary-sqlite`, `PostgreSQL` |
