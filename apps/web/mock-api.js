(() => {
  ...
  const POSTGRESQL_REPOSITORY_ADAPTER_CONTRACT = {
    ...
    version: "postgresql-repository-adapter-v7",
    status: "partial_method_groups",
    stage: "model_registry_audit_organizations_identity_invites_groups_v1",
    present: true,
    ...
    pending_method_count: 15,
    completed_method_group_count: 5,
    completed_method_groups: ["identity_access", "organizations", "staff_invites_delivery", "model_registry", "audit"],
    read_only_method_count: 17,
    read_only_methods: ["get_model_version", "get_active_model_version", "list_model_versions", "list_audit_events", "get_organization", "list_organizations", "get_user", "list_users", "get_user_by_token", "list_active_sessions", "get_staff_invite", "list_staff_invites", "get_staff_invite_delivery_attempt", "get_staff_invite_delivery_event", "list_staff_invite_delivery_attempts", "list_staff_invite_delivery_events", "list_staff_invite_delivery_outbox_attempts"],
    write_method_count: 20,
    ...
    limitation: "PostgreSQL Repository Adapter v7 implements the model registry, audit, organization, identity/session, and staff invite delivery method groups through an injected DB-API compatible connection factory. Runtime backend selection remains disabled until tenant-scoped application, simulation, and analytics flows have repository parity coverage.",
  };
    ...
    return {
      ...
      parity_checks: [
        ...
        },
        {
          ...
          action: "Use the completed organization and staff invite adapter groups as tenant-boundary evidence, then add repository parity tests for MFI queues, analytics, review packets, applications, and simulations.",
        },
        {
        ...
        },
        {
          ...
          status: "pass",
          sqlite_evidence: "52 SQLite repository method(s) are grouped into adapter contract families; 37 method(s) currently have PostgreSQL adapter coverage.",
        ...
        },
        {
          ...
          action: "Use the completed organization adapter group as the tenant boundary foundation before implementing application, invite, simulation, and analytics parity.",
        },
        {
          ...
          action: "Use the completed identity access adapter group as the security foundation before implementing staff invite, application, simulation, and analytics parity.",
        },
        {
          ...
          action: "Use the completed staff invite delivery group as the tenant-scoped access onboarding foundation before application, simulation, and analytics parity.",
        },
        {
          key: "postgresql_repository_backend",
          status: "blocker",
          sqlite_evidence: "Runtime repository supports sqlite only and rejects postgresql at startup; the adapter has complete model registry, audit, organization, identity/session, and staff invite delivery method groups but does not yet cover the full repository contract.",
          postgres_requirement: "Implement a PostgreSQL repository backend behind the same API contract.",
          action: "Expand the PostgreSQL adapter from model registry, audit, organization, identity/session, and staff invite delivery flows to tenant-scoped applications, simulations, and analytics.",
        },
        {
