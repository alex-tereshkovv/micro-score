(() => {
  ...
  const POSTGRESQL_REPOSITORY_ADAPTER_CONTRACT = {
    ...
    completed_method_group_count: 7,
    completed_method_groups: ["identity_access", "organizations", "staff_invites_delivery", "application_lifecycle", "model_registry", "portfolio_analytics", "audit"],
    read_only_method_count: 26,
    read_only_methods: ["get_model_version", "get_active_model_version", "list_model_versions", "list_audit_events", "get_organization", "list_organizations", "get_user", "list_users", "get_user_by_token", "list_active_sessions", "get_staff_invite", "list_staff_invites", "get_staff_invite_delivery_attempt", "get_staff_invite_delivery_event", "list_staff_invite_delivery_attempts", "list_staff_invite_delivery_events", "list_staff_invite_delivery_outbox_attempts", "get_application", "list_applications", "list_borrower_applications", "list_application_decisions", "list_application_timeline", "get_portfolio_simulation", "list_portfolio_simulations", "segment_analytics", "decision_analytics"],
    write_method_count: 26,
    write_methods: ["create_model_version", "activate_model_version", "record_audit_event", "create_organization", "assign_user_organization", "create_user", "disable_user", "reactivate_user", "attest_user_mfa", "create_session", "revoke_session", "revoke_session_by_id", "create_staff_invite", "mark_staff_invite_accepted", "mark_staff_invite_revoked", "mark_staff_invite_delivered", "record_staff_invite_delivery_attempt", "record_staff_invite_delivery_event", "update_staff_invite_delivery_attempt_status", "update_staff_invite_delivery_worker_state", "create_application", "assign_application_organization", "update_application_score", "record_application_decision", "clear_applications", "create_portfolio_simulation"],
    method_groups: [
      {
        key: "identity_access",
    ...
    limitation: "PostgreSQL Repository Adapter v9 implements every SQLite repository method group through an injected DB-API compatible connection factory, including model registry, audit, organization, identity/session, staff invite delivery, application lifecycle, portfolio simulation, and MFI analytics flows. Runtime backend selection remains disabled until a managed PostgreSQL connection, production migration runner, and repository-level disposable PostgreSQL parity CI are implemented.",
  };
    ...
    return {
      ...
      parity_checks: [
        ...
        },
        {
          ...
          action: "Use the completed organization, staff invite, application, and portfolio analytics adapter groups as tenant-boundary evidence while designing optional row-level security.",
        },
        {
        ...
        },
        {
          ...
          status: "pass",
          sqlite_evidence: "52 SQLite repository method(s) are grouped into adapter contract families; 52 method(s) currently have PostgreSQL adapter coverage.",
          ...
          action: "Promote microscore_api.postgres_repository from injected adapter parity into disposable PostgreSQL repository CI.",
        },
        {
        ...
        },
        {
          ...
          status: "pass",
          sqlite_evidence: "2 audit method(s) have PostgreSQL query specs and SQLite-vs-adapter parity coverage.",
          postgres_requirement: "The adapter must preserve append-only audit recording, JSONB details materialization, nullable actor/entity fields, and reverse-chronological review.",
        ...
        },
        {
          ...
          action: "Use the completed organization adapter group as tenant-boundary evidence while promoting repository-level disposable PostgreSQL parity CI.",
        },
        {
          key: "postgresql_identity_access_method_group_adapter",
          status: "pass",
          sqlite_evidence: "11 identity/session method(s) have PostgreSQL query specs and SQLite-vs-adapter parity coverage.",
          postgres_requirement: "The adapter must preserve user registration, staff MFA attestation, disable/reactivate semantics, session TTL filtering, staff-only session views, and session revocation.",
          action: "Use the completed identity access adapter group as security-boundary evidence while promoting repository-level disposable PostgreSQL parity CI.",
        },
        {
          key: "postgresql_staff_invites_delivery_method_group_adapter",
          status: "pass",
          sqlite_evidence: "15 staff invite delivery method(s) have PostgreSQL query specs and SQLite-vs-adapter parity coverage.",
          postgres_requirement: "The adapter must preserve staff invite creation, accept/revoke/deliver semantics, delivery outbox ordering, worker retry/dead-letter fields, idempotent webhook events, and JSONB webhook metadata materialization.",
          action: "Use the completed staff invite delivery group as tenant-scoped access onboarding evidence while promoting repository-level disposable PostgreSQL parity CI.",
        },
        {
          key: "postgresql_application_lifecycle_method_group_adapter",
          status: "pass",
          sqlite_evidence: "10 application lifecycle method(s) have PostgreSQL query specs and SQLite-vs-adapter parity coverage.",
          postgres_requirement: "The adapter must preserve borrower application creation, organization assignment, score/rescore transitions, terminal-state guards, MFI decision workflow, decision history ordering, audit timeline, and reset semantics.",
          action: "Use the completed application lifecycle group as the core loan workflow foundation before promoting portfolio analytics into disposable PostgreSQL repository CI.",
        },
        {
          key: "postgresql_portfolio_analytics_method_group_adapter",
          status: "pass",
          sqlite_evidence: "5 portfolio simulation and analytics method(s) have PostgreSQL query specs plus SQLite-vs-adapter parity coverage.",
          postgres_requirement: "The adapter must preserve Monte Carlo run persistence, organization-scoped simulation history, scored-application segment analytics, latest-decision analytics, policy rows, risk/district/recommendation breakdowns, and proxy-sensitivity buckets.",
          action: "Promote the fully implemented adapter surface into repository-level disposable PostgreSQL parity CI.",
        },
        {
