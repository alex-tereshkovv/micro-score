(() => {
  ...
  const POSTGRESQL_REPOSITORY_ADAPTER_CONTRACT = {
    ...
    version: "postgresql-repository-adapter-v6",
    status: "partial_method_groups",
    stage: "model_registry_audit_organizations_identity_groups_v1",
    present: true,
    ...
    pending_method_count: 30,
    completed_method_group_count: 4,
    completed_method_groups: ["identity_access", "organizations", "model_registry", "audit"],
    read_only_method_count: 10,
    read_only_methods: ["get_model_version", "get_active_model_version", "list_model_versions", "list_audit_events", "get_organization", "list_organizations", "get_user", "list_users", "get_user_by_token", "list_active_sessions"],
    write_method_count: 12,
    write_methods: ["create_model_version", "activate_model_version", "record_audit_event", "create_organization", "assign_user_organization", "create_user", "disable_user", "reactivate_user", "attest_user_mfa", "create_session", "revoke_session", "revoke_session_by_id"],
    method_groups: [
      {
        key: "identity_access",
    ...
    limitation: "PostgreSQL Repository Adapter v6 implements the model registry, audit, organization, and identity/session method groups through an injected DB-API compatible connection factory. Runtime backend selection remains disabled until tenant-scoped application, invite, simulation, and analytics flows have repository parity coverage.",
  };
    ...
    return {
      ...
      parity_checks: [
        ...
        },
        {
          ...
          status: "pass",
          sqlite_evidence: "52 SQLite repository method(s) are grouped into adapter contract families; 22 method(s) currently have PostgreSQL adapter coverage.",
        ...
        },
        {
          ...
          status: "blocker",
          sqlite_evidence: "Runtime repository supports sqlite only and rejects postgresql at startup; the adapter has complete model registry, audit, organization, and identity/session method groups but does not yet cover the full repository contract.",
          postgres_requirement: "Implement a PostgreSQL repository backend behind the same API contract.",
          action: "Expand the PostgreSQL adapter from model registry, audit, organization, and identity/session flows to staff invites, tenant-scoped queues, applications, simulations, and analytics.",
        },
        {
